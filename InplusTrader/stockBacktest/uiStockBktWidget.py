...
########################################################################
class StockValueMonitor(QtGui.QTableWidget):
    """参数监控"""
    signal = QtCore.pyqtSignal(type(Event()))

    #----------------------------------------------------------------------
    def __init__(self, stockBacktestEngine, eventEngine, name, parent=None):
        """Constructor"""
        super(StockValueMonitor, self).__init__(parent)

        self.stockBacktestEngine = stockBacktestEngine
        self.eventEngine = eventEngine
        self.name = name

        self.data = {}
        self.keyCellDict = {}
        self.lables = ['run_id', 'strategy_file', 'start_date', 'end_date', 'stock_starting_cash',
    ...
    def updateData(self, data):
        ...
        if not self.inited:
            if 'run_id' in data.keys():
                ...
                cell = QtGui.QTableWidgetItem(unicode(data['stock_starting_cash']))
                self.resizeColumnsToContents()
...
########################################################################
class StockStrategyManager(QtGui.QGroupBox):
    """策略管理组件"""
    signal = QtCore.pyqtSignal(type(Event()))

    #----------------------------------------------------------------------
    def __init__(self, stockBacktestEngine, eventEngine, name, runID, parent=None):
        """Constructor"""
        super(StockStrategyManager, self).__init__(parent)
        ...
        with open(os.path.abspath(os.path.dirname(__file__)) + '\\config_template.json', 'r') as f:
            ...
            self.setting['base']['strategy_file'] = os.path.abspath(os.path.dirname(__file__)) + '\\strategy\\' + self.name + '.py'
            self.setting['mod']['analyser']['output_file'] = os.path.abspath(os.path.dirname(__file__)) + '\\result\\' + self.name + '\\result.pkl'
            self.setting['mod']['analyser']['plot'] = os.path.expanduser("~")
            self.setting['mod']['analyser']['plot_save_file'] = os.path.abspath(os.path.dirname(__file__)) + '\\result\\' + self.name + '\\result.png'
            self.setting['mod']['analyser']['report_save_path'] = os.path.abspath(os.path.dirname(__file__)) + '\\result\\'
    ...
    def initUi(self):
        ...
        self.baseMonitor = StockValueMonitor(self.stockBacktestEngine, self.eventEngine, self.name)
        self.extraMonitor = StockValueMonitor(self.stockBacktestEngine, self.eventEngine, self.name)
    ...
    def start(self):
        ...
        self.stockBacktestEngine.writeBktLog(self.name + u'策略回测中')
        self.stockBacktestEngine.run(self.config)
        self.stockBacktestEngine.writeBktLog(self.name + u'策略回测成功')

    #----------------------------------------------------------------------
    def show(self):
        """触发显示绩效报告事件（通常用于通知GUI更新）"""
        self.stockBacktestEngine.writeBktLog(self.name + u'绩效报告创建中')
...
class StockBacktestEngineManager(QtGui.QWidget):
    """CTA引擎管理组件"""
    signal = QtCore.pyqtSignal(type(Event()))
    signal2 = QtCore.pyqtSignal(type(Event()))

    #----------------------------------------------------------------------
    def __init__(self, stockBacktestEngine, eventEngine, parent=None):
        """Constructor"""
        super(StockBacktestEngineManager, self).__init__(parent)
        ...
        # 记录日志
        self.stockBacktestEngine.writeBktLog(u'股票回测引擎启动成功')
    ...
    def initStrategyManager(self):
        ...
        for name in self.stockBacktestEngine.strategyDict.keys():
            strategyManager = StockStrategyManager(self.stockBacktestEngine, self.eventEngine, name, self.runID)
    ...
    def load(self):
        ...
        if not self.strategyLoaded:
            self.stockBacktestEngine.loadStrategy()
            self.initStrategyManager()
            self.strategyLoaded = True
            self.stockBacktestEngine.writeBktLog(u'策略加载成功')
        
    #----------------------------------------------------------------------
    def new(self):
        self.stockBacktestEngine.writeBktLog(u'策略新建成功')
    ...
    def updateBktResult(self, event):
        ...
        fileName2 = os.path.abspath(os.path.dirname(__file__)) + '\\result\\' + name + '\\total_portfolios.csv'
        fileName3 = os.path.abspath(os.path.dirname(__file__)) + '\\result\\' + name + '\\stock_portfolios.csv'
        fileName4 = os.path.abspath(os.path.dirname(__file__)) + '\\result\\' + name + '\\stock_positions.csv'
        fileName5 = os.path.abspath(os.path.dirname(__file__)) + '\\result\\' + name + '\\trades.csv'
        ...
        self.stockBacktestEngine.writeBktLog(name + u'绩效报告创建成功')
