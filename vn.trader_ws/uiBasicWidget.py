...
class PriceWidget(QtGui.QWidget):
    ...
    def initUi(self, startDate=None):
        ...
        #self.initHistoricalData()  # 下载历史数据, 默认不显示历史数据， 只显示实时图
    ...
    def onTick(self, tick):
        ...
        hh, mm, ss = tick.time[:-2].split(':')
        self.ticktime = time(int(hh), int(mm), int(ss), microsecond=int(tick.time[-1]))
...
class DAILYMonitor(BasicMonitor):
    ...
    def __init__(self, mainEngine, eventEngine, parent=None):
        """Constructor"""
        super(DAILYMonitor, self).__init__(parent)
        self.mainEngine = mainEngine
        self.eventEngine = eventEngine
...
class MINMonitor(BasicMonitor):
    ...
    def __init__(self, mainEngine, eventEngine, parent=None):
        """Constructor"""
        super(MINMonitor, self).__init__(parent)
        self.mainEngine = mainEngine
        self.eventEngine = eventEngine
...
########################################################################
class TICKMonitor(BasicMonitor):
    """日志监控"""

    #----------------------------------------------------------------------
    def __init__(self, mainEngine, eventEngine, parent=None):
        """Constructor"""
        super(TICKMonitor, self).__init__(mainEngine, eventEngine, parent)
        self.mainEngine = mainEngine
        self.eventEngine = eventEngine
