...
class CtaStrategyManager(QtGui.QGroupBox):
    ...
    #----------------------------------------------------------------------
    def __init__(self, ctaBktEngine, eventEngine, name, parent=None):
        """Constructor"""
    ...
    def updateMonitor(self, event=None):
        """显示策略最新状态"""
        paramDict = self.ctaBktEngine.getStrategyParam(self.name)
        if paramDict:
            self.paramMonitor.updateData(paramDict)
            
        varDict = self.ctaBktEngine.getStrategyVar(self.name)
        if varDict:
    ...
    def init(self):
        """初始化策略"""
        self.ctaBktEngine.initStrategy(self.name)
    
    #----------------------------------------------------------------------
    def start(self):
        """启动策略"""
        self.ctaBktEngine.startStrategy(self.name)
        
    #----------------------------------------------------------------------
    def stop(self):
        """停止策略"""
        self.ctaBktEngine.stopStrategy(self.name)


########################################################################
class CtaBacktestEngineManager(QtGui.QWidget):
    """CTA引擎管理组件"""
    signal = QtCore.pyqtSignal(type(Event()))
    
    #----------------------------------------------------------------------
    def __init__(self, ctaBktEngine, eventEngine, parent=None):
        """Constructor"""
        super(CtaBacktestEngineManager, self).__init__(parent)
        ...
        # 记录日志
        self.ctaBktEngine.writeCtaLog(u'CTA引擎启动成功')        
    ...
    def initUi(self):
        ...
        savePositionButton.clicked.connect(self.ctaBktEngine.savePosition)
    ...
    def initStrategyManager(self):
        ...
        for name in self.ctaBktEngine.strategyDict.keys():
            strategyManager = CtaStrategyManager(self.ctaBktEngine, self.eventEngine, name)
    ...
    def initAll(self):
        """全部初始化"""
        for name in self.ctaBktEngine.strategyDict.keys():
            self.ctaBktEngine.initStrategy(name)    
            
    #----------------------------------------------------------------------
    def startAll(self):
        """全部启动"""
        for name in self.ctaBktEngine.strategyDict.keys():
            self.ctaBktEngine.startStrategy(name)
            
    #----------------------------------------------------------------------
    def stopAll(self):
        """全部停止"""
        for name in self.ctaBktEngine.strategyDict.keys():
            self.ctaBktEngine.stopStrategy(name)
            
    #----------------------------------------------------------------------
    def load(self):
        """加载策略"""
        if not self.strategyLoaded:
            self.ctaBktEngine.loadSetting()
            self.initStrategyManager()
            self.strategyLoaded = True
            self.ctaBktEngine.writeCtaLog(u'策略加载成功')
