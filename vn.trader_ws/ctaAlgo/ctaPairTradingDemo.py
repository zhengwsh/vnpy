...
########################################################################
class PairTradingDemo(CtaTemplate):
    """期货配对交易策略Demo"""
    className = 'PairTradingDemo'
    ...
    def __init__(self, ctaEngine, setting):
        """Constructor"""
        super(PairTradingDemo, self).__init__(ctaEngine, setting)
    ...
    def onInit(self):
        ...
        for vtSymbol in self.vtSymbols:
            initData = self.loadBar(vtSymbol, self.initDays)
            for bar in initData:
                print bar.close
                self.onBar(bar)
        ''' 
    ...
    def onBar(self, bar):
        ...
        if self.counter > 2 * self.window:
            ...
            if bar.vtSymbol == self.vtSymbols[0]:
                self.price_a = bar.close
                pabq = bar.openInterest #self.getPositionLong(self.vtSymbols[0])  # 多头持仓
                pasq = bar.openInterest #self.getPositionShort(self.vtSymbols[0]) # 空头持仓
            if bar.vtSymbol == self.vtSymbols[1]:
                self.price_b = bar.close
